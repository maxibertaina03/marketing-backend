...
describe('AprobacionesService', () => {
  ...
  beforeEach(async () => {
    ...
    const mod = await Test.createTestingModule({
      providers: [
        ...
        { provide: NotificacionesService, useValue: { emitirEvento: jest.fn() } },
      ],
    }).compile();
    ...
    it('devuelve paginado con estado EN_REVISION', async () => {
      ...
      expect(prisma.publicacion.findMany).toHaveBeenCalledWith(
        expect.objectContaining({
          where: expect.objectContaining({ estado: EstadoContenido.EN_REVISION }),
        }),
      );
    });
  });
  ...
  describe('aprobar', () => {
    ...
    it('CLIENTE no puede aprobar publicación de otra marca', async () => {
      prisma.publicacion.findFirst.mockResolvedValue(pubEnRevision);
      await expect(service.aprobar('org1', 'pub1', {}, ctxClienteOtro)).rejects.toThrow(
        ForbiddenException,
      );
    });
    ...
    it('lanza 409 si no está EN_REVISION', async () => {
      prisma.publicacion.findFirst.mockResolvedValue(pubBorrador);
      await expect(service.aprobar('org1', 'pub1', {}, ctxAdmin)).rejects.toThrow(
        ConflictException,
      );
    });
  });
  ...
  describe('rechazar', () => {
    it('mueve EN_REVISION → RECHAZADO y guarda motivo', async () => {
      ...
      prisma.publicacion.update.mockResolvedValue({
        ...
        motivoRechazo: 'Falta imagen',
      });
      await service.rechazar('org1', 'pub1', { motivo: 'Falta imagen' }, ctxAdmin);
      expect(prisma.publicacion.update).toHaveBeenCalledWith(
        expect.objectContaining({
          data: { estado: EstadoContenido.RECHAZADO, motivoRechazo: 'Falta imagen' },
        }),
      );
    });
    ...
    it('CLIENTE no puede rechazar publicación de otra marca', async () => {
      prisma.publicacion.findFirst.mockResolvedValue(pubEnRevision);
      await expect(
        service.rechazar('org1', 'pub1', { motivo: 'x' }, ctxClienteOtro),
      ).rejects.toThrow(ForbiddenException);
    });

    it('lanza 409 si no está EN_REVISION', async () => {
      prisma.publicacion.findFirst.mockResolvedValue(pubAprobada);
      await expect(service.rechazar('org1', 'pub1', { motivo: 'x' }, ctxAdmin)).rejects.toThrow(
        ConflictException,
      );
    });
  });
